    ...
    def ingest_elements(self, url: str, elements: list[dict]):
        ...
        try:
            ...
            for el in elements:
                ...
                content_parts = []
                if el.get("text"):
                    content_parts.append(f"Text/Label: {el['text']}")
                if el.get("role"):
                    content_parts.append(f"Role: {el['role']}")
                if el.get("placeholder"):
                    content_parts.append(f"Placeholder: {el['placeholder']}")
                if el.get("name"):
                    content_parts.append(f"Name: {el['name']}")
                if el.get("id"):
                    content_parts.append(f"ID: {el['id']}")
